import { ViveiroDashboard, HealthScore } from '../types/dashboard';
...
/**
 * Calcula a saúde geral da fazenda baseada na média dos viveiros
 * @param viveiros - Lista de viveiros da fazenda
 * @returns Score geral da fazenda (0-100)
 */
export const calcularSaudeGeral = (viveiros: ViveiroDashboard[]): number => {
  if (!viveiros.length) return 0;
  
  const totalScore = viveiros.reduce((acc, viveiro) => {
    const scoreSaude = calcularScoreSaude(viveiro);
    return acc + scoreSaude.score;
  }, 0);
  
  return Math.round(totalScore / viveiros.length);
};
