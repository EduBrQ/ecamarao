import { ViveiroDashboard, HealthScore } from '../types';
...
export const calcularScoreSaude = (viveiro: ViveiroDashboard): HealthScore => {
  ...
  const biomassaEsperada = viveiro.populacaoEstimada * 0.015;
  ...
  let scoreConsistencia = 15;
  const diasUltimos7 = viveiro.racoes.slice(-7);
  if (diasUltimos7.length >= 5) {
    const mediaConsumo = diasUltimos7.reduce((sum, r) => sum + r.total, 0) / diasUltimos7.length;
    const variacao = diasUltimos7.reduce((sum, r) => sum + Math.abs(r.total - mediaConsumo), 0) / diasUltimos7.length;
    scoreConsistencia = Math.max(0, 25 - (variacao * 5));
  }
  
  // Score Sobrevivência (0-20 pontos)
  let scoreSobrevivencia = 15;
