...
import { preverAlimentacao } from '../services/feedingPredictor';
import { detectarAnomalias } from '../services/anomalyDetector';
import { preverProducao } from '../services/productionForecaster';
...
export const ViveiroCard: React.FC<ViveiroCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="viveiro-previsao">
        <div className="previsao-header">
          <span className="previsao-title">🧠 Previsão Inteligente</span>
          <span className={`previsao-tendencia ${previsaoAmanha.tendencia}`}>
            {previsaoAmanha.tendencia === 'aumento' ? '↑' : '→'} {previsaoAmanha.tendencia === 'aumento' ? 'aumento previsto' : 'estável'}
          </span>
        </div>
        <div className="previsao-grid">
          <div className="previsao-item">
            <span className="previsao-label">Amanhã</span>
            <span className="previsao-value">{previsaoAmanha.racaoHoje.toFixed(1)} kg</span>
          </div>
        </div>
      </div>
      ...
      {/* Alertas Automáticos */}
      {anomalias.length > 0 && (
        <div className="viveiro-alerts">
          <div className="alerts-header">
            <span className="alerts-title">⚠️ Alertas</span>
          </div>
          {anomalias.slice(0, 2).map((anomalia, index) => (
            <div key={index} className={`alert-item ${anomalia.severidade}`}>
              <div className="alert-icon">
                {anomalia.severidade === 'alta' ? '🔴' : '🟡'}
              </div>
              <div className="alert-content">
                <div className="alert-message">{anomalia.mensagem}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="viveiro-producao">
        <div className="producao-header">
          <span className="producao-title">🎯 Produção</span>
        </div>
        <div className="producao-grid">
          <div className="producao-item">
            <span className="producao-label">Peso</span>
            <span className="producao-value">{previsaoProducao.pesoMedioEstimado}g</span>
          </div>
          <div className="producao-item">
            <span className="producao-label">Colheita</span>
            <span className="producao-value">{previsaoProducao.dataColheita.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</span>
          </div>
        </div>
      </div>
